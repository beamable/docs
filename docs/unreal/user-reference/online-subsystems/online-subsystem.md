...
!!! warning "Will Be Discontinued"
    ...
    Beamable has a lot of unique features that are very useful but become a lot harder to use if kept behind the OnlineSubsystem interface. In other words, supporting it is costly but doesn't bring as much value to customers and partners as originally expected. As such, support is being phased out in favor of focusing on the `BeamableCore` plugin, docs, samples, and a showcase project demonstrating how to use Beamable inside UE for a live-services game.
