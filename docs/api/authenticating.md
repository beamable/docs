...
The `X-BEAM-SCOPE` header carries project identification in the format `<cid>.<pid>` — your `cid`, a period, then your `pid`:

```text
X-BEAM-SCOPE: 1434605640884224.DE_1434605640884225
```
...
The `Authorization` header carries the user credential as a [bearer token](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Authorization): the word `Bearer`, a space, then the token itself.

```text
Authorization: Bearer 5f8c9d2e-7b41-4a03-9e6f-1c2d3a4b5c6d
```
...
If you have a player that has an email and password associated with it (such as a developer account), you can retrieve an access token for that account by using the credentials.
...
!!! tip
    This example assumes you have `jq` available. If you do not, just look at the output and find the `accessToken` field.
...
The `X-BEAM-GAMERTAG` header carries the user identification — the player's numeric gamertag:
...
Beamable supports server-to-server privileged API requests in addition to token-based access. In contrast to token-based requests that must act as a particular Beamable user (who may or may not have administrative privileges), signed requests act as a server entity with universal privileges in its realm. A signed request can therefore take server-authoritative actions such as setting `game.private` stats, or submitting scores to a leaderboard whose `writeSelf` access is false.
...
- Signed requests must include a signature in the `X-BEAM-SIGNATURE` header, calculated as described below
- Signed requests may include a player ID in the `X-BEAM-GAMERTAG` header; this is useful for an API route that is normally player-oriented and usually gets the player ID implicitly from the access token
