...
|Name|Type|Description|
|-|-|-|
|--filter|String|A text filter for log searching|
|--server-log-level|String|A log level filter for searching|
|--from|String|A timestamp filter, where logs must be newer than this time. Must be an exact date time string, or a relative time string. Relative time strings are in the format <number><unit>. The unit is either s (seconds), m (minutes), h (hours), or d (days). To represent 5 minutes in the past, use the term '5m' |
|--to|String|A timestamp filter, where logs must be older than this time. Must be an exact date time string, or a relative time string. Relative time strings are in the format <number><unit>. The unit is either s (seconds), m (minutes), h (hours), or d (days). To represent 5 minutes in the past, use the term '5m' |
