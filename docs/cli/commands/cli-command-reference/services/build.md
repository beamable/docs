...
|--force-cpu-arch|Boolean|When true, build an image for the Beamable Cloud architecture, amd64|
|--pull|Boolean|When true, force the docker build to pull all base images|
|--no-cache|Boolean|When true, force the docker build to ignore all caches|
|--tags|String[]|Provider custom tags for the resulting docker images|
|--simultaneous|Boolean|When true, all build images will run in parallel|
