> **Archived 2026-08-19.** This is a historical record of the procedure used to
> migrate the Beamable documentation off ReadMe.io and onto MkDocs Material. The
> migration is finished; nothing here describes current practice, and current
> conventions live in the Style Guide section of `AGENTS.md` on `main`.
>
> It lived as `docs/CONVERSION_MANUAL.md` on eight content branches from
> 2025-10-20 until it was removed from all of them and consolidated here. On those
> branches it was in no `SUMMARY.md` and no `mkdocs.yml`, but MkDocs builds every
> Markdown file under `docs/` regardless of nav, so it published as a reachable,
> search-indexed page on each one. Keeping it on `main`, which builds no site,
> preserves the record without publishing it.
