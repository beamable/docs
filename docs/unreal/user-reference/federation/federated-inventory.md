...
- Whenever the inventory is fetched, if that content object is within its scope, the Beamable backend will invoke the `GetInventoryState` call with the user's 3rd Party  `UserId` as its parameter.
