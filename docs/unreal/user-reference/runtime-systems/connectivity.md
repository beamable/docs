...
After a user is logged-into a `FUserSlot`, losing the connection to the Beamable backend (due to internet access loss or otherwise), will have the `UBeamConnectivityManager` go into the `CONN_Offline` state. The details of this process are:
...
While in `CONN_Offline` mode, we'll keep trying to reestablish the `FUserSlot`'s connection with Beamable. This happens automatically in the background and is a continuous process. If we manage to reestablish the connection, **_one of two_** things will happen:
...
- If `Project Settings -> Beam Runtime -> AutomaticallyNotifyFixupComplete` is `false`:
    - We'll broadcast the `UBeamConnectivityManager::OnReconnected` delegate.
    - We'll go into the `CONN_Fixup` state and set up the `UBeamConnectivityManager::FixupTick` function.
...
For these reasons, we decided to **_NOT_** automatically refresh and instead to **_give you the tools to correctly set up your game state as you need it to be_**. Therefore: 
