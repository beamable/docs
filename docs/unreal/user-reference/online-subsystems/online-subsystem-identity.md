...
- Use the `IOnlineIdentity::Login` function with a `LoginType` of  `BeamOSS::GetIdentityTypeFederated()` with `UOnlineSubsystemBeamableSettings::bAutoSignUpWhenLogin` as `true`.
    - Each third-party identity may require different inputs. This depends on how your federation is set up. You can see our [Discord](../../samples/discord-demo.md) and [Steam](../../samples/beamball/steam-integration.md) samples for examples of how to write these federations.
