...
Before you can use the Beamable CLI, you need to complete the [Getting-Started Guide](doc:cli-guide-getting-started). That means having [.NET 8](https://dotnet.microsoft.com/en-us/download/dotnet/8.0) installed, and getting the  [Beam CLI](https://www.nuget.org/packages/Beamable.Tools).
...
To pipe the `data` to a file, run:
```sh
...
To print only the `cid` value to console, run:
```sh
dotnet beam config | jq '.data.cid'
"123"
```

Sometimes it is important to get the unescaped JSON, so the `fromjson` component of JQ can be used. For example, to get the `cid` value without quotes:
