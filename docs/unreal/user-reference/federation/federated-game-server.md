...
The **Create Game Server** function is a blocking call with a long default timeout. This means Beamable's Backend waits on a response before notifying the users they were put into the lobby. However, implementing this correctly for all cases can be challenging --- as such, the SDK provides a few utilities:
