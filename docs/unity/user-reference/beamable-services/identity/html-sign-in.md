...
In the HTML above, the Login button refers to a JavaScript function that does not yet exist, namely `login()`. Let us use a bit of jQuery to retrieve the username and password values from the form and then pass them along to the `/basic/auth/token` endpoint.
