...
## Version 2.0.1
This is the release notes for the Beamable Unreal SDK version 2.0.1.

This release has several fixes regarding increasing support for macOS as a development platform, fixing certain dedicated server (linux) issues as well as updating the backing CLI version for additional fixes within the microservice code-generation pipeline.

**Follow the [Upgrade Steps](../getting-started/setup.md#upgrading-the-sdk)**.

Then do the following:

- If you are upgrading from `1.X.X`, follow any instructions in the [version 2.0.0 upgrade guide](#version-200)
- To fix a bug, this hotfix requires that you regenerate your microservice clients after rebuilding the microservice projects, if you have any. Run: `dotnet beam project generate-client "."`
...
- Added an override to the path to `dotnet` (found in `Project Settings -> Beam Editor`) so that non-default `dotnet` setups can be used
    ...
    - After changing this setting, restart the editor
...
- Fixed issue where `CallingContext` was not being passed along in `Authenticate` calls in certain edge-cases
...
## Version 2.0.0
This is the release notes for the Beamable Unreal SDK version 2.0.0

This is a major release in our release cycle, it brings a lot of new features and improvements to the Beamable Unreal SDK support and user experience in general including a new Content Editor, a complete overhaul of our blueprint nodes, support for Friends and Parties, new support for Android and many others.
...
The Beamable Unreal SDK now supports Unreal Engine 5.5.x as it minimum version. Older versions of Unreal Engine are no longer officially supported. If you are using an older version of Unreal Engine, we recommend you upgrade to 5.5.x to take advantage of the latest features and improvements or stay in the 1.x versions of the Beamable Unreal SDK.

#### Android support
![The Android robot mascot logo with the "android" wordmark beneath it.](../../media/imgs/android.png)

The Beamable Unreal SDK now supports Android builds officially, allowing you to build and deploy your games on Android devices. This includes support for the Beamable services, as well as the ability to use the Unreal Engine's native Beamable features.
...
![The reworked CLI-backed Content window listing content types such as BeamCurrencyContent and BeamGameTypeContent, with the currency.coins object selected and its properties in the right-hand detail panel.](../../media/imgs/contentv2-window.png)
...
#### Support for friends and parties
The Beamable Unreal SDK now supports the [Friends](../user-reference/beamable-services/friends.md) and [Parties](../user-reference/beamable-services/parties.md) features of Beamable. This includes the ability to send and receive friend requests, manage your friends list, and create and join parties.

#### Blueprint nodes rework
The Blueprint nodes have undergone a comprehensive overhaul to enhance consistency and ease of use. Updates include clearer node naming, refined input and output types, and significant improvements to overall usability. Additionally, we have introduced a [dedicated section](../user-reference/runtime-systems/blueprints.md) to help users better understand how to effectively utilize Blueprints with the Beamable Unreal SDK.

![An On Clicked event on a Purchase button wired into an Operation - Store - Perform Purchase node with a Store Id of stores.SkinStore and a Listing Id input, exposing On Success, On Error, On Cancelled, and operation-event output pins.](../../media/imgs/blueprint-operators.png)
...
This federation is guaranteed to run only once per player, whenever the player is created in the realm. On the Beamable Unreal SDK side, all SignUp functions expose the `InitProperties` parameter which is a map that will be passed along to the `IFederatedPlayerInit` in the microservice. This can be used to implement custom sign-up flows and even special sign-up cases that provide players a better starting state should your game benefit from such a design.

#### Microservice client code generation
In the previous versions of the SDK, you could set `<GenerateClientCode>true</GenerateClientCode>` in the `.csproj` of your Microservices to generate the client code for them on every rebuild.

This is no longer possible.

We made this change for a few reasons:

- For projects with multiple microservices, a solution-wide rebuild triggers one generation process per-project rebuild. As each code-gen must take into account ALL services due to Unreal's namespacing limitations, this is not always desirable and can cause an entire class of complex problems
- It would trigger at inconvenient times (that is, with the UE editor open) which could sometimes cause UE-related issues
...
In this version of the SDK, this is a manual process. Run the following command line from inside your Unreal project's directory whenever you wish to generate your latest client code. You can also add a run configuration in your IDE if you prefer to just press a button to trigger the regeneration.
...
- All non-frictionless `SignUp` functions now include auto-login: if the identity is already in use, they automatically attempt to log in with it instead of failing
- CLI is now optionally installed — none of the beamable editor functionality will be available and the project will always set the TargetRealm to the branch-wise realm; not the local override
    - If you want to have a subset of your team work without the ability to swap realms and/or interact with Beamable's editor workflows at all; you no longer need to install the CLI for that subset
    - In most cases and for most teams, we recommend you **do not** do this and instead just have .NET and the CLI installed for everyone though
- `UBeamContentObject` now supports `FText` usage with `StringTable`

#### CLI/Microservices - upgrade instructions
If you were using the `FederatedGameServer` interface, you will also have to do this.
...
We made a few changes to our authentication flow to make realm changing a faster and simpler process. In light of that, you will need to re-authenticate with the CLI. So, run `dotnet beam login` and re-authenticate into your organization.
