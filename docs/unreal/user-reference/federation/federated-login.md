...
2. \[**Game Client**]: Invoke a `Login`/`SignUp`/`Attach` **Operation** and pass the following parameters:
	...
	2. **IdentityUserId**: this is the third party's `UserId` for the user trying to login. Beamable uses this to determine if there is already a Beamable account mapped to this third-party Id
...
- Use the `Sign-Up - Federated Identity` node with `Auto Login` passing in:
    ...
    - The Federation's Id
    - The User Id _**of the Federated third-party user**_ (this would be the user's Steam Id, for example)
    - A token that can be used to authenticate this user's account with the Federated third party

It looks like this:

![A Blueprint graph in two labelled stages: "Initialize the Beamable SDK" running Beam Runtime initialization, and "Set up Federated Login Via Steam" passing the Steam credentials into a Login - Federated Identity operation.](../../../media/imgs/steam-demo-login.png)
...
1. Use the third-party APIs or C# SDKs to validate the provided `token`
2. Use the third-party APIs or C# SDKs to get the `UserId` for that `token`'s user
...
For non-MFA flows (which are most of the Store and Console login flows) this is all that is needed. Here is an example from the [Steam Demo](../../samples/steam-demo.md).
...
Here is an example from the Sui-Wallet integration showcase.
