...
This is because initializing the SDK is an Asynchronous Process and takes time — so there is no way to tell Unreal to wait until the SDK is initialized to then run `Begin Play`. From `PostLogin` forward, you can use the SDK; Content is ready, the Lobby information is available and so on...
