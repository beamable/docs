...
- `UBeamRequestTracker`: provides BP-compatible async operations that emit various events as they go along
    ...
    - This is a somewhat complex subsystem so read [Operations and Waits](operations/operations-and-waits.md) for more details.<br><br>
...
If you ever encounter issues with this system, `log LogBeamBackend Verbose` is a useful Unreal diagnostic command. It will print out the entire process of building the request, sending it out and receiving its response.
...
This WebSocket connection semantically represents the connectivity status for that particular user. The SDK (and the Beamable Servers) consider a user online or offline based on whether this connection is alive and well. So if you want to use any of the SDK's real-time services such as [Matchmaking](../beamable-services/matchmaking.md) and [Lobbies](../beamable-services/lobbies.md), this connection needs to be working properly. At runtime, `UBeamRuntime`, and `UBeamConnectivityManager` handle per-`FUserSlot` [Connectivity](connectivity.md) statuses.
