...
Beamable publishes a set of [OpenAPI](https://swagger.io/specification/) documents, including [beamable-oapi.json](https://raw.githubusercontent.com/beamable/spec/refs/heads/main/beamable-oapi.json). These documents are used to generate C# classes and methods inside the Beamable SDK. Access this SDK through the `IDependencyProvider` available via a `BeamContext` instance.

The data structures used for requests and responses are JSON serializable by Unity, which means you can view them in the Unity Inspector. As new Beamable server features are released, the autogenerated SDK will capture the latest features.
...
Access the autogenerated SDK via the `ServiceProvider` on any `BeamContext` instance. The SDK is a set of interface types for each Beamable service. All interfaces and model types are under the `Beamable.Api.Autogenerated` namespace.
...
Most Beamable APIs require user authorization. The generated SDK functions for these APIs will not include any authorization options, because authorization is required. Every request sent through the auto generated SDK will use the authorization from the associated `BeamContext`.

However, some Beamable APIs do not require user authorization. If authorization is not required, the SDK function will include a boolean parameter, `includeAuthHeader`. The parameter will always default to `true`, but it is possible to override the default behavior and pass in `false`. Sending a request without an authorization header will have different effects depending on the SDK being executed.
