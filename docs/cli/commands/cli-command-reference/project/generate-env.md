...
|--exclude-otel-creds|Boolean|If true, do not ask for otel auth credentials to be put in the env|
...
|--include-secret|Boolean|When enabled, includes the legacy SECRET realm secret environment variable|
|--use-remote-deps|Boolean|When enabled, prevents any connection strings from being added to the environment|
|--remove-all-except-pid|Int32|When enabled, automatically stop all other local instances of this service|
