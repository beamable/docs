...
The Blueprint nodes have undergone a comprehensive overhaul to enhance consistency and ease of use. Updates include clearer node naming, refined input and output types, and significant improvements to overall usability. Additionally, we've introduced a dedicated section in the documentation to help users better understand how to effectively utilize Blueprints with the Unreal SDK.

## Android Support
The Unreal SDK now supports Android builds, allowing you to build and deploy your games on Android devices. This includes support for the Beamable services, as well as the ability to use the Unreal Engine's native Beamable features.
