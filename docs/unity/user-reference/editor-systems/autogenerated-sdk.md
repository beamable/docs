...
However, some Beamable APIs do not require user authorization. If authorization is not required, the SDK function will include a boolean parameter, `includeAuthHeader`. The parameter defaults to `true`, but you can pass `false` to send the request without an authorization header. Sending a request without an authorization header will have different effects depending on the SDK being executed.
