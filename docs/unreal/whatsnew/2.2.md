...
Previously, our custom configurations were included in Unreal’s default configuration files:

- `DefaultGame.ini`
- `DefaultEngine.ini`
- `DefaultEditor.ini`

Now, these settings have been moved into separate files with the **"Beam"** prefix, allowing for a clearer separation between base project settings and Beam-specific settings.
...
If you already had custom settings in your `.ini` files, you'll need to copy them into the new files. Follow these steps:

1. **Open your old files**:  
   - `DefaultGame.ini`  
   - `DefaultEngine.ini`  
   - `DefaultEditor.ini`

2. **Copy only the sections related to Beam** (or any other custom settings we've moved).

3. **Paste the content into the corresponding new files**:
   - Content from `DefaultGame.ini` → `DefaultBeamGame.ini`
   - Content from `DefaultEngine.ini` → `DefaultBeamEngine.ini`
   - Content from `DefaultEditor.ini` → `DefaultBeamEditor.ini`

4. **Save the files** and ensure they are properly included in your version control system.
