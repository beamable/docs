...
**Three live branches publish nothing on push:** `api/v1.0`, `home`, and `websdk/v1.0` carry no `auto-publish-branch.yml`, so they update only by a manual `Deploy Docs Branch` dispatch. The script reports `no deploy registered` for them and moves on rather than waiting out its timeout. `home` additionally carries no `auto-sync-core.yml` stub, unlike every other non-core branch.
