...
Pass these via the `Options` parameter.
...
The `BeamMultiplayer::Authentication::PreLoginAsync` function runs an operation that, once completed successfully, informs you that the SDK has validated the user is both who they say they are and are in the lobby they say they are. You can do game-specific logic here and combine it with the SDK's verification too.
...
```c++
/**
 * If your game does not have multiple local players, this is not needed — it does not hurt, though.
...
Implement the snippet below and configure your custom Game Instance class in `Project Settings > Project > Maps & Modes > Game Instance Class`.
