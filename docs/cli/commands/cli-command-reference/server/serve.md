...
|--custom-splitter|Boolean|When true, will use custom logic to split the command line given to the server via HTTP request.<br>The default splitter (from Microsoft) does NOT allow you to pass in JSON blobs as arguments.<br>The custom splitter does its best to support all our commands correctly and accept json blobs as arguments|
