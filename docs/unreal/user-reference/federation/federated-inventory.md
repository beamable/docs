...
- Whenever the inventory is fetched, if that content object is within its scope, the Beamable backend will invoke the `GetInventoryState` call with the user's third-party `UserId` as its parameter.
...
- The "transaction" parameter is an optional string provided by the caller. You are expected to provide an idempotency guarantee that every given transaction string value is only processed once. In most cases, this is just a pass-along to Web3 wallets or wherever you're keeping the inventory information. If that's not supported by your third-party, you can use StorageObjects to provide this guarantee
