...
![The PIE tab's Player Manager listing captured players on the left, with the selected player's Player Data — GamerTag, CID, PID, access token, and expiration — and Save as Local, Save as Template, and Delete User actions on the right.](../../../media/imgs/pie-user.png)
...
![The PIE tab's Play Presets view with a preset selected, showing its General Settings, an Override Multiplayer Settings section with a Game Type, and a User Settings section assigning a player to a PIE client.](../../../media/imgs/pie-settings.png)
...
![A preset's General Settings showing its Settings ID and Name, an empty Map Name Pattern field, and an Available Maps list with an Add Map button.](../../../media/imgs/pie-map-settings.png)
...
![The Override Multiplayer Settings section enabled by its header checkbox, exposing a Server Map Override dropdown, a Game Type set to game_types.dynamic_match, and a Lobby Global Data list with an Add Data button.](../../../media/imgs/pie-fake-lobby.png)
...
![A User Settings section with two PIE clients, each with User Slot, Player, and Party dropdowns; the first is in Party_1 on TeamA with Party Settings exposing an Is Party Leader toggle, the second is in No Party.](../../../media/imgs/pie_play_users.png)
