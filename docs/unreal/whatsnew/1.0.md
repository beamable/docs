...
We recommend you start this process with no staged changes to your VCS and in an isolated local branch such that you can revert as needed.
...
- Open a shell (we recommend `GitBash` if you are using `git`) in your project root directory
...
!!! warning "Access Denied"
   ...
   Contact our customer support if the above does not work.

If the script is successful, you should see something like this:
...
# If you are using OnlineSubsystemBeamable, you should see these lines...
...
- If you want, re-enable Automatic Code Generation by setting `<GenerateClientCode>true</GenerateClientCode>` in every service's `csproj` file
  - If you do not want, you can read about manual generation [here](../user-reference/microservices/microservices.md)
...
- `Target.cs` file and `Beam.Configure___` functions
	...
	- Before, you had to manually add the various `BeamableCore` modules to these things. Now, you can call the respective functions based on the Type of `Target` or `Module`
	- You can refer to the [intro guide](../getting-started/setup.md#setting-up-modules) to see which functions can be called in which `Target`/`Modules`
...
- Instead of `RegisterOnStarted`/`RegisterOnReady`, you should explicitly invoke: `InitSDK` or `InitSDKWithFrictionlessAuth`
	- `RegisterOnReady` no longer exists. Its semantics were hard to explain and provided little benefits
	  ...
	  - We removed `IsFirstAuth` and `bDidFirstAuthRun` in favor of a simpler `IsLoggedIn(FUserSlot)` function with which you can build
	    - The new semantics are slightly different and just tell you that there is a user in that slot or not; you are free to handle "Return to main menu" situation as you wish
	  - Read more about the initialization flow [here](../user-reference/overview.md#beamable-runtime-sdk)
...
- If you have not already, regenerate the microservice clients by rebuilding them (or running the `dotnet beam generate-client` command)
	...
	- About code-changes:
	    ...
	    - You will have to go through the code and use the newly generated types
