...
|----------|-------------|
| **Tournament ID** | Create and name new content in Unity using the Tournament ID. This is a content ID in the tournaments content type: a typical tournament ID looks like `tournaments.weekly_ascension` where the part after the dot is meaningful to you, the Game Maker. |
...
| **Rank Rewards** | These rewards reference the Beamable Inventory for fulfillment. The system evaluates and grants these rewards at the end of each Tournament cycle. Players must explicitly claim rewards via the Tournament API for fulfillment. |
