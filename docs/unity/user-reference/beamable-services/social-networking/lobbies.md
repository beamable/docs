...
The Beamable **Lobbies** service requires a name, a restriction type (either `Open` or `Closed`), and a `gameTypeId` or [`SimGameType`](https://csharp.cdocs.beamable.com/latest/classBeamable_1_1Common_1_1Content_1_1SimGameType.html#details) which is a Beamable ContentObject for Multiplayer when creating lobbies.
