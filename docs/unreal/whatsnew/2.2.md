...
### Bug Fix: Code Generation Pipeline for Microservices and Low-Level Services
We identified and fixed an issue in the **code generation pipeline** that affected both **microservices** and **low-level services** in the latest release.
To apply the fix and regenerate the client code correctly, please run the following command in your project directory:
```bash
