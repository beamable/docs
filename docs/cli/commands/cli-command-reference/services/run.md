...
|-|-|-|
|--force-amd-cpu-arch|Boolean|[DEPRECATED] Force the services to run with amd64 CPU architecture, useful when deploying from computers with ARM architecture|
|--ids|String[]|The ids for the services you wish to deploy. Ignoring this option deploys all services|
|--force-local-cpu|Boolean|By default, this command forces the services to run with amd64 CPU architecture, which is the architecture used in Docker|
|--keep-containers|Boolean|Automatically remove service containers after they exit|
