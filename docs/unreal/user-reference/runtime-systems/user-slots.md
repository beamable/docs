...
The UBeamUserSlots subsystem does not handle the actual logging in and logging out. Instead, that is handled by a `GameInstanceSubsystem` called `UBeamRuntime` which is responsible for PIE instances and packaged games.
