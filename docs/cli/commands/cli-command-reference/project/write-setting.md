...
```shell
beam project write-setting <beamoId> [options]
```
...
|Name|Type|Description|
|-|-|-|
|--skip-build|Boolean|If options are modified, the project needs to be re-built so that the embedded resource is available on the next execution of the service. However, this build operation may be skipped when this option is set|
|--key|Set[String]|The keys of the settings. The count must match the count of the --value options|
|--delete-key|Set[String]|The keys of the settings to be deleted. If the key is also set in a --key option, the key will not be deleted|
|--value|Set[String]|The values of the settings. The count must match the count of the --key options|
