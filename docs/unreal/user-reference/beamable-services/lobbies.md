...
To read from the `GlobalData` you can use any of the following nodes:

![Three Local State - Lobby global-data nodes: iterate all key/values, retrieve one value with a default fallback, and deserialize a JSON blob into a cast target implementing IBeamJsonSerializableUObject.](../../../media/imgs/lobbies-read-global-data.png)

To read from a specific player's data, you can use any of these nodes:

![The equivalent Local State - Lobby player-data nodes: iterate all key/values, retrieve one value with a default, and deserialize a JSON blob into a cast target.](../../../media/imgs/lobbies-read-player-data.png)
...
Here is the list of events exposed:
...
![A collection of Local State - Lobby helper nodes for mapping Unreal gameplay-framework constructs such as Player Controllers and Player States to Beamable User Slots and GamerTags.](../../../media/imgs/lobbies-gameplay-helpers.png)
