...
```shell
beam telemetry set-config <cli-log-level> <cli-telemetry-max-size> <cli-allow-telemetry> [options]
```
...
|cli-telemetry-max-size|String|The maximum size in bytes for saved Otel data|
|cli-allow-telemetry|Boolean|Define if the telemetry is allowed to push informations to beamable server|
