...
```shell
beam project generate-client-oapi [options]
```
...
|--server|Boolean|Instructs the generator to write code for a server, not a client|
|--output-dir|String|Directory to write the output client at|
|--oapi-path|String|Path to an existing beam_openApi.json file; skips manifest lookup when provided|
