...
![Local State - Auth nodes for IsLoggedIn, GetOwnerSlotConnectivity, and GetSlotConnectivity, with their return values feeding Beam Connectivity Manager checks such as Is Disconnected, Is Authenticated, and Has Never Connected This Session.](../../../media/imgs/connectivity-local-state-nodes.png)
...
1. `UBeamConnectivityManager::OnConnectionLost` fires once (per-connection loss)
...
!!! warning "Why not do Request-based Heuristics?"
    We have tried estimating internet connectivity via heuristics over failed requests.
    We have found that a more stable connectivity semantic produces better results.
    Requests time out if you are not connected to the internet, or if they are dispatched but cannot reach the Beamable servers.
