...
- **Video over animated GIF.** GIF has no interframe compression; a 16-second screen capture costs ~23 MB as GIF and under 1 MB as H.264. Convert with `ffmpeg -i in.gif -movflags +faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -c:v libx264 -crf 26 -preset slow -an out.mp4`. Animated WebP is not a substitute — measured at 85% of the GIF lossless, 16% lossy, against H.264's 4%. Choose the markup by what the clip is for: a decorative banner whose motion is the point keeps autoplay with `<video autoplay loop muted playsinline>` (muted is mandatory or browsers block it); a walkthrough someone follows along with uses `<video controls preload="metadata" poster="...">`, so it does not loop underneath the reader and costs nothing until played. Always give a `poster` frame for the click-to-play form, and put the description GIF alt text used into a `<figcaption>`
