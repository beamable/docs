...
- Third-party platform integration (Steam, Epic, Discord, Web3 wallets, etc) with support for implementing Multi-Factor Auth
...
> Authenticating the user, fetching all relevant data from the backend into the `UBeamRuntimeSubsystem`'s in-memory state, and then completing the operation.
...
In the success handling of the operation, the UI is told to update itself. While not shown in this image, that UI can then use the [`Local State - Stats`](stats.md) and [`Local State - Inventory`](inventory.md) nodes to read the player's stats and inventory to render itself.
